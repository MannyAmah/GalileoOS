...
3. **`galileo-agent-runner` (Go).** Connects to Temporal, registers workflows via an explicit lookup map (`workflowRegistry := map[string]workflowFunc{"hello": HelloAgentWorkflow}` — not convention-based dispatch; explicit lookups survive rename refactors that conventions don't). Stage 0 ships one workflow (`HelloAgentWorkflow`) and one activity (`CallLLMActivity`); the activity calls the gateway, gateway calls LiteLLM, response returned. Cost metadata flows back into the workflow result via `AgentOutput.cost_event_request_ids` (proto schema change introduced in PR-B alongside cost_events writing). Ships in PR-C.
4. **`galileo-web` (Next.js).** Single page at `:3001` that triggers the Hello Agent workflow, polls for completion, displays the response + the cost. Ships in PR-C. ~150 LOC.
...
7. **Observability — trace emission.** Every gateway request emits a span; every workflow execution shows as one parent span with N LLM children. Stage 0 acceptance test runs the demo 100× and confirms 100 parent spans in the observability backend. **Original plan named Opik specifically; the Week 3 inline-plan round's discovery pass surfaced that Opik's self-hosted deployment is a 13-container platform (MySQL + Redis + ClickHouse + Zookeeper + MinIO + 5 Opik-specific services + Jaeger + OTel collector), not a single observability service comparable in weight to Postgres or LiteLLM.** The substrate decision is deferred to PR-B's planning round, where it gets the proper plan-deviation treatment (closeout + plan edits + ADR + code change) if a substitution is chosen. PR-A ships gateway with structured stdout logging only; PR-B owns observability wiring.
